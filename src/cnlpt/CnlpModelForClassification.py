...
class CnlpModelForClassification(PreTrainedModel):
    ...
    def compute_loss(
        ...
        if task_num_labels == 1:
            ...
            if self.class_weights[task_ind] is not None:
                class_weights = torch.FloatTensor(self.class_weights[task_ind]).to(
                    self.device
                )
            else:
                class_weights = None
            ...
            if self.relations[task_ind]:
                task_labels = labels[
                    :, 0, state["task_label_ind"] : state["task_label_ind"] + seq_len, :
                ]
        ...
        if state["loss"] is None:
            ...
            task_weight = (
                1.0 if task_ind + 1 < len(self.num_labels) else self.final_task_weight
            )
    ...
    def apply_arg_reg(
        ...
        prob_no_e1_type = relu(
            torch.sign(probs_e1[:, :, 0].unsqueeze(2) - probs_e1[:, :, 1:]).sum(dim=2)
            - t1_threshold
        )
        prob_no_e2_type = relu(
            torch.sign(probs_e2[:, :, 0].unsqueeze(2) - probs_e2[:, :, 1:]).sum(dim=2)
            - t2_threshold
        )
    ...
    def generalize_encoder_forward_kwargs(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        for name, value in kwargs.items():
            if name not in params and value is not None:
                ...
                logger.warning(
                    f"Parameter {name} not present for encoder class {self.encoder.__class__.__name__}."
                )
            elif name in params:
